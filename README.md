...
- Der Spurversatz der KI-Autos richtet sich jetzt nach der echten Fahrbahnbreite aus den Straßendaten (`verkehr.json`, Feld `w`) statt nach einem für jede Straße gleichen Festwert - auf schmalen Straßen bleibt entsprechend weniger Platz zum Ausweichen, ohne über den Fahrbahnrand hinauszufahren.
...
- Kein echtes Abbiegen an Kreuzungen: Jedes KI-Auto fährt ausschließlich seine eine zugewiesene Straße vor und zurück (siehe `Tools/Export/prepare-verkehr.cjs`) - Kreuzungen wirken nur auf Vorfahrt/Anhalteabstand, nicht auf die Streckenwahl selbst. Echtes Abbiegen auf eine kreuzende Straße bräuchte eine Routenplanung über den Straßengraphen (mehrere Straßensegmente zu einer Fahrt verketten, Fahrspurwechsel beim Einbiegen) - eine deutlich größere, eigenständige Funktion, nicht nur eine Geometrie-Verfeinerung der bestehenden Kreuzungslogik.
