...
- **Skelett-Mesh nur bei einer von 90 KI-Passanten-Figuren, nur eine Kleidung:** Ein lizenziertes, echt geriggtes CC0-Modell (Quaternius, siehe „Datenquellen und Lizenzen“) mit echter Idle-/Walk-Animation ersetzt jetzt den Kasten-Rig - aber nur die eine importierte Figur ("Farmer"), alle KI-Passanten sehen also gleich aus. Die Spielfigur bleibt beim von Hand gebauten Gelenk-Rig (`LaLaBergKoerperTeile.h`, keine UAnimSequence, keine Fußauftritts-Erkennung) - der rechte Arm haelt die Waffe in einer festen Pose, ohne sich beim Zielen zu beugen. Waffen und Fahrzeuge sind eigene, lizenzierte Meshes (CC0/CC BY 4.0). Kein Missionssystem.
